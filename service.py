...
def train(id, model, reqdata):
    ...
    if(response["status"]==2):
        ...
        data_connector.uploadModelPackage(id,'package','package.zip',response["status"],response["reason"],response["numRecords"],endTime)
    else:
        data_connector.updateStatus(id,response["status"],response["reason"],response["numRecords"],endTime)
    #update model status
